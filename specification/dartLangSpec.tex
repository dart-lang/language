...
\begin{itemize}
\item
  An expression that denotes an invocation or
  % Could a property extraction (i.e., a tear-off) be allowed? We would
  % need to support capturing a different `this` for each invocation of the
  % enclosing constructor in cases like `const C(Object arg): x = arg.m`
  % when `arg.m` is an extension method tear-off. That's a composite term
  % giving rise to multiple constant values, which is new---e.g., we don't
  % allow `const C(arg): x = [arg]`.
  a property extraction
  of an extension member
  is not potentially constant, and not constant.
  %% TODO(eernst): Come extension methods, delete this and add \ref{}.
  \commentary{%
    A future version of this language specification will specify
    extensions, their members, extension member invocations,
    and the mechanism for property extraction of such members.%
  }
