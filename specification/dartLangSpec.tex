...
%% TODO(eernst): When landing PR #534, preserve the following 2 paragraphs.
\LMHash{}%
An implementation is allowed to invoke \code{loadLibrary}
one or more times on all deferred prefixes
before \code{main}
(\ref{scripts})
is invoked.

\commentary{%
This means that an implementation can treat deferred imports
essentially as if they had been immediate
(as long as it is all of them or none of them, in the whole program),
but it must still be possible to invoke \code{loadLibrary}
and get the same behavior as if the library had already been loaded
due to a user-written invocation of \code{loadLibrary}.%
}
