...
\item[$\bullet$]
  ...
  perform 32-bit operations on those, and the resulting 32 bits are
  interpreted as a 32-bit unsigned integer. For example,
  \code{-1\,\ltlt\,1} evaluates to 4294967294. The right shift
  operator, \lit{\gtgt}, performs a signed shift when the sign bit of
  the JavaScript number is 1, and an unsigned shift when it is 0. For
  example, with one in bits 1 and 31 and zeros elsewhere the shift is
  unsigned in \code{0x80000002\ \gtgt\ 1\ ==\ 0x40000001}, and signed in
