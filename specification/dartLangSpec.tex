...
\item
  ...
  \end{itemize}
\item
  \textbf{Left FutureOr (\SrnLeftFutureOr, \SrnNullTwo):}
...
\item
  ...
  if $S$ is \code{$S_0$?} then \SubtypeNE{S}{T} if{}f
  \SubtypeNE{S_0}{T} and \SubtypeNE{\code{Null}}{T}.
\item
  \textbf{Type Variable Reflexivity 1 (\SrnLeftPromotedVariableOne):}
  if $S$ is a type variable $X_0$
  or a promoted type variable \code{$X_0$\,\&\,$S_0$} and $T$ is $X_0$
  then \SubtypeNE{S}{T}.
...
\item
  ...
  and $T$ is \code{$X_0$\,\&\,$S_1$}
  then \SubtypeNE{S}{T} if{}f \SubtypeNE{S}{S_1}.
