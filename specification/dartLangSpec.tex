...
%   derive all kinds of member declarations (for a class, a mixin, etc),
...
and multiplicative (\ref{multiplicativeExpressions}) operators.
...
\newcommand{\SrnLeftNullable}{8}
\newcommand{\SrnLeftPromotedVariableOne}{9}
\newcommand{\SrnRightPromotedVariable}{10}
\newcommand{\SrnRightFutureOrA}{11}
\newcommand{\SrnRightFutureOrB}{12}
\newcommand{\SrnRightNullableOne}{13}
\newcommand{\SrnRightNullableTwo}{14}
\newcommand{\SrnLeftPromotedVariableTwo}{15}
\newcommand{\SrnLeftVariableBound}{16}
\newcommand{\SrnRightFunction}{17}
\newcommand{\SrnPositionalFunctionType}{18}
\newcommand{\SrnNamedFunctionType}{19}
\newcommand{\SrnCovariance}{20}
\newcommand{\SrnNominal}{21}
...
\begin{figure}[p]
  ...
  \begin{minipage}[c]{0.49\textwidth}
    ...
    % ------------------------------------------------ Right Object
    % TODO(eernst): Come records, treat \RECORD{} the same as \FUNCTION.
  ...
  \begin{minipage}[c]{0.49\textwidth}
    ...
    \Rule{\SrnLeftPromotedVariableOne}{X}{T}{X \& S}{T}
    % ------------------------------------------------ Right FutureOr A
...
in section~\ref{subtypes} and its subsections
is assumed to have no compile-time errors,
...
\LMHash{}%
The type of a function object is
...
It works as if \FUNCTION{} were removed from the \IMPLEMENTS{} clause
...
\commentary{%
  ...
  \SubtypeNE{\code{FutureOr<$T$>}}{\code{Object?}},
  and also that \code{FutureOr} is covariant in its type parameter,
  just like class types:
  if \SubtypeNE{S}{T} then
  \SubtypeNE{\code{FutureOr<$S$>}}{\code{FutureOr<$T$>}}.%
}
...
In the rules, every type under consideration is a static type.
...
\item
  ...
  whose statically known type is \VOID{} may have type \VOID.
  \rationale{%
...
\item
  In a list or set literal
  (\ref{collectionLiterals})
  $C$ whose actual type argument is \VOID{},
  an element at the top level in $C$ may have element type \VOID{}.
  In a map literal $M$ whose first actual type argument is \VOID{},
  an element at the top level in $M$ may have key type \VOID{}.
  In a map literal $M$ whose second actual type argument is \VOID{},
  an element at the top level in $M$ may have value type \VOID{}.
  \commentary{%
    For example, if \code{v} has type \VOID{} then
    \code{<\VOID>[v]} and \code{<int, \VOID>\{1:\,\,v\}} are allowed.
    This rule is only concerned with top-level elements, that is,
    elements which are not nested inside other elements.
    Nesting of elements is handled via the following rule.%
  }
\item
  The expression in an \synt{expressionElement} may have type \VOID{}.
  \commentary{%
    The element type of the expression element is then \VOID{}, too,
    which may or may not be an error in the given context.
    The same is true for the other kinds of elements
    which are covered below.%
  }

  In a \synt{mapElement} of the form \code{$e_1$:\,\,$e_2$},
  $e_1$ may have type \VOID{} and $e_2$ may have type \VOID{}.
  In a \synt{nullAwareMapElement} of the form \code{?$e_1$:\,\,$e_2$},
  $e_2$ may have type \VOID{}.
  In a \synt{nullAwareMapElement} of the form \code{$e_1$:\,\,?$e_2$},
  $e_1$ may have type \VOID{}.

  In an \synt{ifElement} $I$ of the form
  \code{\IF\,\,($c$)\,\,$e_1$} or \code{\IF\,\,($c$)\,\,$e_1$\,\,\ELSE\,\,$e_2$},
  $e_1$ may have element type, key type, or value type \VOID{},
  and so may $e_2$.

  In a \synt{forElement} $F$ of the form
  \code{\FOR\,\,(\metavar{forLoopParts})\,\,$e$}, $e$ can have
  element type, key type, or value type \VOID{}.

  \commentary{%
    Consider a \synt{spreadElement} $S$
    of the form \code{...$e$} or \code{...?$e$}.
    Assume that $e$ has a static type that implements
    \code{Iterable<$T$>}, \code{Map<$T$, $S$>}, or \code{Map<$S$, $T$>}
    for some types $S$ and $T$.
    In the case where $T$ is \VOID, $S$ will have \VOID{}
    as its element, key, or value type.
    This may or may not be an error in the given context.
    On the other hand, it is an error if $e$ has type \VOID.
    So there is no need for a rule about spread elements here.

    Note there is no exception for a \synt{nullAwareExpressionElement},
    and for some locations in a \synt{nullAwareMapElement}.
    The reason is that these element kinds will use the expression value,
    and hence it is an error if the expression has type \VOID.%
  }
\item
...
because it is concerned with relationships among program elements
that may occur in different binding environments.
...
\item $S$ is an identifier or a prefixed identifier which is resolved
  ...
  \commentary{%
    ...
    and it also covers type variables.%
  }
...
\item (1) $S$ and $T$ are both positional function types with the same
  ...
  Otherwise (2), $S$ and $T$ are both named function types with the same
...
\LMHash{}%
For the purpose of the above ruleset, \code{Object}, \code{dynamic},
...
\begin{itemize}
\item
  \textbf{Reflexivity (\SrnReflexivity):}
  if $T_0$ and $T_1$ are the same type (\ref{sameType})
  then \SubtypeNE{T_0}{T_1}.

  \commentary{%
    Note that this check is necessary as the base case for atomic types
    and for type variables, but not for composite types.
    In particular, a structural equality check is admissible,
    but not required here.
    Pragmatically, non-constant time equality checks here are
    counter-productive.
    Hence, this rule is only used when $T$ is atomic.%
  }
\item
  \textbf{Right Top (\SrnRightTop):}
  if $T$ is \code{Object?}, \DYNAMIC, or \VOID{} then \SubtypeNE{S}{T}.
\item
  \textbf{Left Top (\SrnLeftTop):}
  if $S$ is \DYNAMIC{} or \VOID{}
  then \SubtypeNE{S}{T} if{}f \SubtypeNE{\code{Object?}}{T}.
\item
  \textbf{Left Bottom (\SrnBottom):}
  if $S$ is \code{Never} then \SubtypeNE{S}{T}.
\item
  \textbf{Right Object (\SrnLeftVariableBound*, \SrnLeftPromotedVariableTwo*, %
    \SrnLeftFutureOr*, \SrnRightObjectFour):}
  if $T$ is \code{Object} then:
  \begin{itemize}
  \item
    if $S$ is an unpromoted type variable with bound $B$
    then \SubtypeNE{S}{T} if{}f \SubtypeNE{B}{\code{Object}}.
  \item
    if $S$ is a promoted type variable \code{$X$\,\&\,$S$}
    then \SubtypeNE{S}{T} if{}f \SubtypeNE{S}{\code{Object}}.
  \item
    if $S$ is \code{FutureOr<$U$>} for some $U$,
    then \SubtypeNE{S}{T} if{}f \SubtypeNE{U}{\code{Object}}.
  \item
    if $S$ is \code{Null}, \DYNAMIC, \VOID, or \code{$S$?} for any $S$,
    then the subtyping does not hold
    (\commentary{%
      i.e., the result of the subtyping query is known to be false%
    }).
  \item
    Otherwise \SubtypeNE{S}{T} is true.
  \end{itemize}
\item
  \textbf{Left Null (\SrnRightFutureOrB*, \SrnNullOne):}
  if $S$ is \code{Null} then:
  \begin{itemize}
  \item
    if $T$ is a type variable (promoted or not) the query is false.
  \item
    if $T$ is \code{FutureOr<$U$>} for some $U$,
    then the query is true if{}f \SubtypeNE{\code{Null}}{U}.
  \item
    if $T$ is \code{$U$?} for some $U$ then the query is true.
  \item
    Otherwise, the query is false.
  \end{itemize}
\item
  \textbf{Left FutureOr (\SrnLeftFutureOr):}
  if $S$ is \code{FutureOr<$U$>}
  then \SubtypeNE{S}{T} if{}f
  \SubtypeNE{\code{Future<$U$>}}{T} and \SubtypeNE{U}{T}.
\item
  \textbf{Left Nullable (\SrnLeftNullable):}
  if $S$ is \code{$U$?} then \SubtypeNE{S}{T} if{}f
  \SubtypeNE{U}{T} and \SubtypeNE{\code{Null}}{T}.
\item
  \textbf{Type Variable Reflexivity 1 %
    (\SrnLeftPromotedVariableOne, \SrnReflexivity*):}
  if $S$ is a type variable $X$
  or a promoted type variable \code{$X$\,\&\,$U$} and $T$ is $X$
  then \SubtypeNE{S}{T}.
...
\item
  \textbf{Type Variable Reflexivity 2 (\SrnRightPromotedVariable*):}
  if $S$ is a type variable $X$
  or a promoted type variable \code{$X$\,\&\,$U$}
  and $T$ is \code{$X$\,\&\,$V$}
  then \SubtypeNE{S}{T} if{}f \SubtypeNE{S}{V}.

  \commentary{%
    Note that this rule is admissible, and can be safely elided if desired.%
  }
\item
  \textbf{Right Promoted Variable (\SrnRightPromotedVariable):}
  if $T$ is a promoted type variable \code{$X$\,\&\,$U$}
  then \SubtypeNE{S}{T} if{}f \SubtypeNE{S}{X} and \SubtypeNE{S}{U}.
\item
  \textbf{Right FutureOr (\SrnRightFutureOrA, \SrnRightFutureOrB, %
    \SrnLeftVariableBound*, \SrnLeftPromotedVariableTwo*):}
  if $T$ is \code{FutureOr<$U$>}
  then \SubtypeNE{S}{T} if{}f any of the following hold:
  \begin{itemize}
  \item either \SubtypeNE{S}{\code{Future<$U$>}}.
  \item or \SubtypeNE{S}{U}.
  \item or $S$ is $X$ and $X$ has bound $B$ and \SubtypeNE{B}{T}.
  \item or $S$ is \code{$X$\,\&\,$V$} and \SubtypeNE{V}{T}.
  \end{itemize}
\item
  \textbf{Right Nullable (\SrnRightNullableOne, \SrnRightNullableTwo, %
    \SrnLeftVariableBound*, \SrnLeftPromotedVariableTwo*):}
  if $T$ is \code{$U$?}
  then \SubtypeNE{S}{T} if{}f any of the following hold:
  \begin{itemize}
  \item either \SubtypeNE{S}{U}.
  \item or \SubtypeNE{S}{\code{Null}}.
  \item or $S$ is $X$ and $X$ has bound $B$ and \SubtypeNE{B}{T}.
  \item or $S$ is \code{$X$\,\&\,$V$} and \SubtypeNE{V}{T}.
  \end{itemize}
\item
  \textbf{Left Promoted Variable (\SrnLeftPromotedVariableTwo):}
  If $S$ is a promoted type variable \code{$X$\,\&\,$U$}
  then \SubtypeNE{S}{T} if{}f \SubtypeNE{U}{T}.
\item
  \textbf{Left Type Variable Bound (\SrnLeftVariableBound):}
  If $S$ is a type variable $X$ with bound $B$
  then \SubtypeNE{S}{T} if{}f \SubtypeNE{B}{T}.
\item
  \textbf{Function Type/Function (\SrnRightFunction):}
  $S$ is a function type and $T$ is \FUNCTION.
%% TODO(eernst): Come records, include this rule, and add an
%% inference rule for this in the 'Subtype rules' figure:
%% \item
%%   \textbf{Record Type/Record ():}
%%   $S$ is a record type and $T$ is \RECORD.
\item
  \textbf{Covariance (\SrnCovariance):}
  If $S$ is an interface type \code{$C$<$S_1$, \ldots, $S_s$>}
  and $T$ is \code{$C$<$T_1$, \ldots, $T_s$>}
  then \SubtypeNE{S}{T} if{}f \SubtypeNE{S_i}{T_i} for each $i$.
\item
  \textbf{Nominal subtyping (\SrnNominal):}
  If $S$ is an interface type with super-interfaces \List{S}{0}{n}
  then \SubtypeNE{S}{T} if{}f \SubtypeNE{S_i}{T} for some $i$.
\item
  \textbf{Positional Function Types (\SrnPositionalFunctionType):}
  If $S$ is
  ...
  then \SubtypeNE{S}{T} if{}f each of the following criteria is satisfied,
  ...
  \end{itemize}
\item
  \textbf{Named Function Types (\SrnNamedFunctionType):}
  If $S$ is
  ...
  then \SubtypeNE{S}{T} if{}f the following criteria are all satisfied,
...
%% TODO(eernst): Come records, uncomment the following:
% \item
%   If $S$ is \code{($V_0$, \ldots, $V_n$, \{$V_{n+1} d_{n+1}$, \ldots, $V_m d_m$\})}
%   and $T$ is \code{($S_0$, \ldots, $S_n$, \{$S_{n+1} d_{n+1}$, \ldots, $S_m d_m$\})}
