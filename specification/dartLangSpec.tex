...
\LMHash{}%
It is a compile-time error if a yield statement appears in
a function that is not a generator function.

\LMHash{}%
Let $T_e$ be the static type of $e$ and
let $f$ be the immediately enclosing function.
\commentary{Let $T_f$ specify the type of elements that $f$ yields:}
If the body of $f$ is marked \code{\ASYNC*} and
the declared return type of $f$ implements \code{Stream<$U$>} for some $U$
(\ref{interfaceSuperinterfaces})
then let $T_f$ be $U$.
Otherwise, if the body of $f$ is marked \code{\SYNC*} and
the declared return type of $f$ implements \code{Iterable<$U$>} for some $U$
then let $T_f$ be $U$.
% $f$ is a generator function, so the return type must be a supertype of
% `Iterable<U>`/`Stream<U>` for some `U`, so here it can only be a top type.
Otherwise let $T_f$ be \DYNAMIC.
...
\LMHash{}%
Let $T$ be the static type of $e$ and
let $f$ be the immediately enclosing function.

\LMHash{}%
\commentary{Let $T_e$ specify the type of elements that $e$ provides:}
If the body of $f$ is marked \code{\ASYNC*} and
$T$ implements \code{Stream<$U$>} for some $U$
(\ref{interfaceSuperinterfaces})
then let $T_e$ be $U$.
Otherwise, if the body of $f$ is marked \code{\SYNC*} and
$T$ implements \code{Iterable<$U$>} for some $U$
then let $T_e$ be $U$.
%% TODO(eernst): Come nnbd, change 'a top type' to \DYNAMIC{}.
Otherwise if $T$ is a top type then let $T_e$ be \DYNAMIC.
Otherwise a compile-time error occurs.
