...
if an attempt is made to instantiate $A$ (such as \code{new A(42)}),
and we do not want the checker to complain about unimplemented methods in $A$.
In contrast, if $A$ is intended to be concrete,
the checker should raise an error for each member which is unimplemented,
but clients should be allowed to create instances.
...
(\ref{interfaceInheritanceAndOverriding}),
and $m$ is not a correct member override of $m'$
(\ref{correctMemberOverrides}).

\commentary{%
Note that other, somewhat similar kinds of compile-time errors may exist as well
...
an accessible member signature $m''$ with the same name as $m$,
such that $m''$ has a parameter $p''$ that corresponds to $p$
(\ref{covariantParameters}),
and the type of $p$ is not a subtype and not a supertype
of the type of $p''$.
...
\commentary{%
A getter declaration may conflict with other declarations as well
(\ref{classMemberConflicts}).
...
\commentary{%
A constructor declaration may conflict with static member declarations as well
(\ref{classMemberConflicts}).%
}
...
\commentary{%
Static method declarations may conflict with other declarations as well
(\ref{classMemberConflicts}).%
}
