...
Language versioning allows tools and compilers of an SDK to associated exactly one language version with *each library* in a program.

It does so by:

* Assigning a default language version to each Dart package.
* Assigning each Dart library to at most one Dart package.
* Make each library inherit its package’s default language version unless it specifies a library-specific language version override.
* Default to the most recent language version if there is no other hint.
