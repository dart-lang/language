...
**Bob: This is unclear, since the SDK constraint specifies a *range* of
versions. I think what this should do is that the *minimum* version of the
constraint determines the default language level all code in the package can
assume. If a Dart SDK tool's version is not within the constraint at all, that's
an error.**
...
**Bob: Should tools make it an error for a library to select a *higher* version?
Doing so will always cause problems because users of older SDKs could end up
with this library but not be able to use it.**
...
**Bob: This looks nice, but to the lexer, that's a floating point literal. It would be Very Bad if an implementation couldn't tell "2.2" and "2.20" apart. Should we quote it "2.2" just to be safe?**

**I think we should allow omitting the library name since it's mostly useless. I'd use a nameless library as the example here. I don't think "with" reads very well. It also sounds like it's opting in to a newer version, when really it's more like clamping to a historic version. Maybe:**
...
**Bob: Reusing the shebang would cause problems if you also wanted to have a shebang line. -1**
...
**Bob: A couple of nice things about this:**

* It means we can evolve this syntax itself without breaking older versions of
  Dart. Given the nature of this feature, it might be nice to try to insulate it
  in that way.
* It's trivial for tools to insert a new line comment without interfering with
  existing code.
* It's easily greppable.
* Doing something a little ugly might gently encourage migration. :)

**If we did this, I'd suggest something longer and unambiguous like "//@use dart
2.2".**
...
**Bob: +100. This, to me, is the key part of the proposal. There is a Dart train
and it is moving. You can slip a couple of cars back for a while, but you are
expected to be on the train. You cannot unlink and freely reorder the cars for
...
Unpackaged libraries (libraries with `file:` or `http:` URIs, anything except `package:` and `dart:`) cannot use this approach to get a different default. Unpackaged libraries include *tests* in pub packages.

**Bob: Tests *are* part of the containing package as far as the rest of the
system is concerned. They cannot be *imported* from other packages, but they are
definitely part of the package. We can just put the default language version in
the .packages file and use that for libraries not under lib. Pub can add that
line based on the SDK constraint.**

Tools will get a `--default-package` flag that allows users to set a package that all unpackaged libraries are considered as belonging to. This should be used when running tests or other pub-package related files that are not in the Dart package. (If Dart ever gets a notion of package-privacy, this feature will allow tests to pierce the privacy of their own package, without providing a general way for code to do so).
...
**Bob: One concern is that package authors are disincentivized from migrating.
To migrate, they logically must raise the minimum SDK constraint, but that in
turn makes it impossible for users on older SDKs to use their package.
Hopefully, the new features are delightful enough to mitigate this, but we
...
**Bob: Given my above comment about the .packages file, can we skip this part?**
...
**Bob: +1. Based on some discussions we've had, I think of a major version
release as just removing the ability to opt *out* of features we've already
previously shipped. So the minor versions leading up to it let you gradually opt
in to everything, and the major version just permanently flips the bit on for
them all.**
...
**Bob: So far, dartfmt (and the analyzer API it uses) has been able to work fine
as the superposition of all versions. For example, you never had to pass an
experiment flag to dartfmt for optional new or set literals. It just treated
those as always on. I don't know if this will be true for all language
changes.**
...
**Bob: Conversely, if we ship a non-breaking change in the same version as a
breaking change, there's no way to get to the non-breaking feature without
opting in to the breaking one too and doing the migration. We may wish to keep
that in mind when deciding which features to bundle into releases.**
...
**Bob: Would `int.parse()` allow underscores in strings too? If so, this is a
more complex change.**
